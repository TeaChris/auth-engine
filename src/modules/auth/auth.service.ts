...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    const user = await this.authRepository.create({ email: dto.email, password: hashedPassword });
  ...
  async login(dto: LoginDto) {
    ...
    await cache.set(`${REFRESH_PREFIX}${user.id}`, refreshToken, REFRESH_TTL_SEC);
  ...
  async refreshTokens(incomingToken: string) {
    ...
    await cache.set(`${REFRESH_PREFIX}${userId}`, newRefresh, REFRESH_TTL_SEC);
  ...
  async logout(refreshToken: string): Promise<void> {
    try {
      ...
      this.logger.info({ userId: payload['sub'] }, 'User logged out');
    } catch {
